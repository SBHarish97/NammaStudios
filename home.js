window.addEventListener("load", () => {
    ...
    lines.forEach((line, index) => {
        ...
        setTimeout(() => {
            ...
            line.style.transition = "1s cubic-bezier(.22,1,.36,1)";
...
(function heroPreview() {
  ...
  function getCursorDot() {
    return document.getElementById("customCursorBox");
  }
...
const hoverStack=document.getElementById("hoverStack");
...
images.forEach((src,i)=>{
    ...
    img.style.transform=`rotate(${(i-2)*4}deg)`;

    img.style.zIndex=i;

    hoverStack.appendChild(img);

});

const words=document.querySelectorAll(".u");
...
function follow(){
    ...
    hoverStack.style.left=cx+"px";
    hoverStack.style.top=cy+"px";
...
words.forEach(word => {
    ...
    let hoverStackTimer = null;

    word.addEventListener("mouseenter", () => {

        clearTimeout(hoverStackTimer);
        hoverStackTimer = setTimeout(() => {
            hoverStack.style.display = "block";
            document.getElementById("customCursorBox").style.opacity = "0";
            startAnimation();
        }, 200);
    ...
    word.addEventListener("mouseleave", () => {
        ...
        clearTimeout(hoverStackTimer);
        hoverStack.style.display = "none";
         document.getElementById("customCursorBox").style.opacity = "1";
        stopAnimation();    
...
const imgs = [...hoverStack.querySelectorAll("img")];
    ...
    timer = setInterval(()=>{
        ...
        if(current >= imgs.length){
            ...
            imgs.forEach((img,i)=>{
                ...
                img.style.opacity = "0";
                img.style.transform = `translate(0px,0px) rotate(0deg) scale(.8)`;
        ...
        img.style.transform = `
        translate(${current*18}px,${current*12}px)
        rotate(${(Math.random()*14)-7}deg)
        scale(1)
        `;

        img.style.zIndex = current + 20;
...
const playObserver = new IntersectionObserver((entries) => {
    ...
    entries.forEach(entry => {
        ...
        if(entry.isIntersecting){
            ...
            }, [...playCards].indexOf(entry.target) * 180);
...
playCards.forEach(card => playObserver.observe(card));
function stopAnimation(){
    ...
    imgs.forEach(img=>{
        ...
        img.style.transform="translate(0,0) scale(.8)";
...
(function statusBar() {
  ...
  function render() {
    ...
    el.textContent = `${city.label}  ${time}`;
  }
...
(function projects() {
  ...
  const projectData = [
    {
      title: "Matera",
      tag: "Le syndic du 21e siècle",
      video: "https://studionamma.vercel.app/videos/matera/matera-highlight-2.mp4",
      url: "https://studionamma.com/work/matera"
    },
    {
      title: "Chance",
      tag: "Rencontrer l'amour pro",
      video: "https://studionamma.vercel.app/videos/chance/chance-cover.mp4",
      url: "https://studionamma.com/work/chance"
    },
    {
      title: "Silvr",
      tag: "Supercharge business loan",
      video: "https://studionamma.vercel.app/videos/silvr/silvr-highlight.mp4",
      url: "https://studionamma.com/work/silvr"
    },
    {
      title: "Intramuros",
      tag: "Magazine",
      video: "https://studionamma.vercel.app/videos/intramuros/intramuros-highlight.mp4",
      url: "https://studionamma.com/work/intramuros-magazine"
    }
  ];

  track.innerHTML = projectData
    .map(
      (p) => `
      <a class="project-card" href="${p.url}" target="_blank" rel="noopener">
        <video src="${p.video}" muted loop playsinline autoplay></video>
        <span class="project-tag">${p.tag}</span>
        <span class="project-label">${p.title}</span>
      </a>`
    )
    .join("");
...
(function services() {
  ...
  if (!list || !preview || !wrapper) return;

  const serviceData = [
    { title: "Art direction", sub: "Storytelling", video: "https://cdn.prod.website-files.com/679cb9cacf00799ba4b4c985/68bef694221ab31dc7a0898e_Home-artdirection-transcode.webm", start: 0 },
    { title: "Branding", sub: "Identity", video: "https://cdn.prod.website-files.com/679cb9cacf00799ba4b4c985/68bef76a17912168bc67811f_Home-branding-transcode.webm", start: 3 },
    { title: "Webflow", sub: "Integration", video: "https://cdn.prod.website-files.com/679cb9cacf00799ba4b4c985/68bef7ea54539b4433a39276_Webflow-home-transcode.webm", start: 5 },
    { title: "UI/UX Design", sub: "Wireframes", video: "https://cdn.prod.website-files.com/679cb9cacf00799ba4b4c985/68bef86a1717db6af24e06f2_UI-UX-Home-transcode.webm", start: 2 },
    { title: "GSAP animations", sub: "Transitions", video: "https://cdn.prod.website-files.com/679cb9cacf00799ba4b4c985/69f31d237bb976aa6e0bc834_Scene-1_webm.webm", start: 4 },
    { title: "Advertising", sub: "Creative ads", video: "https://cdn.prod.website-files.com/679cb9cacf00799ba4b4c985/68df9e1b94554da67438cec0_Ads-service-homeV3-transcode.mp4", start: 6 },
    { title: "SEO & Content", sub: "Ranking", video: "https://cdn.prod.website-files.com/679cb9cacf00799ba4b4c985/68befca9c05567e63bc4d4a0_seo%20content%20home-transcode.webm", start: 1 }
  ];

  list.innerHTML = serviceData
    .map(
      (s, i) => `
      <li class="service-row${i === 0 ? " active" : ""}" data-index="${i}">
  ...
  preview.innerHTML = serviceData
    .map((s, i) => `<video data-index="${i}" data-start="${s.start}" src="${s.video}" muted loop playsinline${i === 0 ? " autoplay" : ""}></video>`)
    .join("");
  ...
  function highlightRow(row) {
    ...
    const top = rowRect.top + rowRect.height / 2 - preview.offsetHeight / 2;
    const clampedTop = Math.max(12, Math.min(top, window.innerHeight - preview.offsetHeight - 12));
    preview.style.top = clampedTop + "px";
  }

  function swapVideo(row) {
  ...
  function seekAndPlay(video) {
    const startAt = parseFloat(video.dataset.start) || 0;

    const seek = () => {

      video.currentTime = video.duration ? Math.min(startAt, video.duration - 0.1) : startAt;
      video.play();
    };
  ...
  rows.forEach((row) => {
    row.addEventListener("mouseenter", () => {
      ...
      clearTimeout(rowShowTimer);
      clearTimeout(videoTimer);

      rowShowTimer = setTimeout(() => {
  ...
  window.addEventListener("resize", () => {
    const activeRow = list.querySelector(".service-row.active");
    if (activeRow) highlightRow(activeRow);
  });
})();
...
(function cta() {
  ...
  const MATERA_LOGO = `<svg class="client_logo is-matera" viewBox="0 0 162 28" fill="none" xmlns="http://www.w3.org/2000/svg">
    <mask id="mask0_2019_1800" style="mask-type:luminance" maskUnits="userSpaceOnUse" x="0" y="0" width="162" height="28">
  ...
  const CHANCE_LOGO = `<svg class="client_logo is-chance" viewBox="0 0 160 28" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  const QONTO_LOGO = `<svg class="client_logo is-qonto" viewBox="0 0 128 38" fill="none" xmlns="http://www.w3.org/2000/svg">
    <mask id="mask0_2019_1807" style="mask-type:luminance" maskUnits="userSpaceOnUse" x="0" y="0" width="128" height="38">
  ...
  const logosHTML = `
    <span class="cta-logo cta-logo--svg cta-logo--qonto">${QONTO_LOGO}</span>
    <span class="cta-logo cta-logo--svg cta-logo--matera">${MATERA_LOGO}</span>
    <span class="cta-logo cta-logo--svg cta-logo--chance">${CHANCE_LOGO}</span>
    <span class="cta-logo cta-logo--svg cta-logo--silvr">${SILVR_LOGO}</span>
    <span class="cta-logo cta-logo--svg cta-logo--alan">${ALAN_LOGO}</span>
  `;
  ...
  revealObserver.observe(section.querySelector("h2"));
})();